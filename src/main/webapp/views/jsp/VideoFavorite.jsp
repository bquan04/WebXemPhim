...
<div class="controller" style="width: 80%; margin: 0 auto;">
	...
	<div
		...
		<div>
			<a class="btn "
				href='<c:url value="/VideoFavorite?action=true"></c:url>'
				style="background-color: #F45050;"> <i class='bx bxs-heart'></i>
				Phim Được Yêu Thích
			</a>
			 <a class="btn btn-light" 	href='<c:url value="/VideoFavorite?action=false"></c:url>'
				>
				<i class='bx bxs-sad'></i>Phim Chưa Yêu Thích
			</a>
		</div>
	</div>
	...
	<table class="table">
		...
		<tbody>
			<c:choose>
				<c:when test="${not empty listVieoFavorite }">
					<c:forEach items="${listVieoFavorite }" var="item"
						varStatus="status">
						<tr>
							<th>${status.index }</th>
							<td style="font-weight: bold; font-style: italic;">${item.getTitle() }</td>
							<td><img alt=""
								src="/Web-OOP1312/images/${item.getPoster() }"
